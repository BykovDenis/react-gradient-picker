...
class ReactGradientPicker extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.applyPickerGradientParams = this.applyPickerGradientParams.bind(this);
  }
  ...
  updatePointsColor(activeElement, step, data, newDataCustom = false) {
    const { state } = this;
    let newData;
    if (!newDataCustom) {
  ...
  changeColorActiveElement(key, color, alpha, step) {
    ...
    const { state } = this;
    const updatedData = this.updatePointsColor(activeElement, step, state.data);
  ...
  removePointElement(element) {
    const { state  } = this;
    return state.data.filter(elem => elem.step !== element.step);
  }
    ...
    return (evt) => {
      ...
      if (state.activeElement) {
        ...
        if (activeElementSign && keyActiveElement > 0 && keyActiveElement < state.data.length - 1) {
          ...
          const posY = this.getPosY(evt.clientY);
          const widthContainer = this.gradientContainer.clientWidth;
          let stepPoint = this.pixelsToPercent(posX, widthContainer);
          let data = state.data;
          activeElement = {
          ...activeElement,
            stepPoint,
          ...
          if (posY <= props.heightPoint) {
            console.log(posY);
            data = sortCollectionGradient(this.removePointElement(activeElement));
            const lastElement = data[data.length - 1];
            const { key, step, color, alpha, active } = lastElement;
            activeElement = {
              key,
              step,
              color,
          ...
          const updatedData = this.updatePointsColor(activeElement, stepPoint, data);
          this.setState({
            ...state,
            ...this.gradientProperies,
            data: sortCollectionGradient(updatedData.newData),
  ...
  applyPickerGradientParams() {
    this.props.changePaletteParam(this.state.exportData);
  }

  render() {
    const { props, state } = this;
    const signActivityButton = state.activeElement.active;
    const buttonsContainerStyle = signActivityButton
      ? props.classes.buttonsContainer
      : props.classes.buttonsContainerActive;
    return (
      <MuiThemeProvider theme={theme}>
        ...
        />
        <ToggleDisplay if={props.visibleGradientCSS}>
          <Card>
...
ReactGradientPicker.defaultProps = {
  gradient: [
    {step: "0", color: "ffffe5", alpha: 1},
    {step: "0.1", color: "f7fcb9", alpha: 1},
    {step: "0.2", color: "d9f0a3", alpha: 1},
    {step: "0.3", color: "addd8e", alpha: 1},
    {step: "0.4", color: "78c679", alpha: 1},
    {step: "0.5", color: "41ab5d", alpha: 1},
    {step: "0.6", color: "238443", alpha: 1},
    {step: "0.7", color: "006837", alpha: 1},
    {step: "1", color: "004529", alpha: 1}
  ],
  visibleGradientCSS: true,
  heightGradientPanel: 25,
  widthPoint: 15,
  heightPoint: 15,
  changePaletteParam: (gradientStyle) => {
...
export default withStyles(styles)(ReactGradientPicker);