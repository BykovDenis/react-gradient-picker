...
function encodeGradientProperties(colorsProperties) {
  ...
  if (colorsProperties) {
    ...
    Object.keys(colorsProperties).forEach(function (index) {
      ...
      var alphaColor = (0, _convertAlphaParameter.convertAlphaHEXParameterToDec)(alpha);
      data.push({
        key: i,
        step: percentStep,
        color: rgbaColor,
      ...
      var parsedColor = 'rgba(' + rgbaColor.r + ', ' + rgbaColor.g + ', ' + rgbaColor.b + ', ' + alphaColor + ')';
...
function decodeGradientProperties(colorsProperties, maxStep) {
  return Object.keys(colorsProperties).map(function (index) {
    ...
    var alphaColor = (0, _convertAlphaParameter.convertAlphaDecParameterToHEX)(parseFloat(alpha, 10));
